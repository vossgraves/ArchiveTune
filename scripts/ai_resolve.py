...
1. FORK FEATURES MUST SURVIVE. The fork adds: Tidal music source (package `tidal`, TidalAudioProvider, TidalInstanceHealthManager, TidalAccountManager, TidalSettings, TidalCookieUtils), a multi-source audio framework (package `audiosource`: Deezer/Amazon providers, IsrcResolver, AudioSourceConfig, and `resolveMultiSourceDataSpec` wired in MusicService.kt), and secret-based release signing (workflows require GitHub Secrets KEYSTORE/KEY_ALIAS/KEYSTORE_PASSWORD/KEY_PASSWORD and must never fall back to a committed keystore). Never produce a resolution that deletes, disables, or unwires these, and never reintroduce a committed-keystore signing fallback.
