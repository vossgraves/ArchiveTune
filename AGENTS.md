...
- **Playback client policy**: `AutoChoosePlaybackClientKey` + `YTPlayerUtils` own bounded YouTube client selection/fallback and video-scoped PO tokens; the extractor tiers they fall back through live in `playback/stream/` and are ordered in `ResolveAudioStreamUseCase.fallbackTiers` and nowhere else. Manual mode must remain available.
