...
object SourceCheckService {
    ...
    private suspend fun checkTidal(context: Context): SourceCheckResult {
        ...
        val healthyInstances = runCatching {
            app.atf.media.tidal.TidalInstanceHealthManager.healthyUrls(context).size
        }.getOrDefault(0)
    ...
    private suspend fun checkQobuz(context: Context): SourceCheckResult {
        ...
        val healthLabel = when (health) {
            app.atf.media.tidal.TidalAudioProvider.InstanceHealth.HEALTHY -> "healthy (premium)"
            app.atf.media.tidal.TidalAudioProvider.InstanceHealth.PREVIEW_ONLY -> "preview-only (no subscription)"
            app.atf.media.tidal.TidalAudioProvider.InstanceHealth.UNREACHABLE -> "unreachable (token invalid / app_secret mismatch)"
            else -> "unknown"
        }
        return SourceCheckResult(
            healthy = health == app.atf.media.tidal.TidalAudioProvider.InstanceHealth.HEALTHY,
            summary = "Pool accounts: ${accounts.size} ($premium premium)\n" +
                "First token probe: $healthLabel\n" +
                "Qobuz source is ${if (health == app.atf.media.tidal.TidalAudioProvider.InstanceHealth.HEALTHY) "READY" else "NOT ready — see above"}.",
        )
    }
